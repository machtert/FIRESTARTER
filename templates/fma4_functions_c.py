...
#                    file.write("    for (i = 0; i<"+str(buffersize)+"; i++) ((double*)addrMem)[i] = 0.25 + (double)(i&0x1FFF) * 0.27948995982e-4;\n")
                    ...
                    for i in range(0,len(each.instr_groups)):
                        ...
                        if each.instr_groups[i] == 'REG':
                            flops=8 # two 128 bit FMA operations
                        elif each.instr_groups[i] == 'L1_L':
                            flops=12 # one 128 bit and one 256 bit FMA operation
                            ...
                            flops=4 # one 128 bit FMA operation
                ...
                for threads in each.threads:
                    ...
                    for i in range(0,util.repeat(sequence,lines)):
                        for item in sequence:
                            if item == 'REG':
                                d0_inst   = 'vfmaddpd %%xmm'+str(add_start+(add_dest-add_start+add_regs+1)%add_regs)+', %%xmm0, %%xmm'+str(add_dest)+', %%xmm'+str(add_dest)+';'
                                d1_inst   = 'vfmaddpd %%xmm'+str(add_start+(add_dest-add_start+add_regs+2)%add_regs)+', %%xmm1, %%xmm'+str(mov_dst)+', %%xmm'+str(mov_dst)+';'
                                d3_inst   = 'xor %%'+str(shift_reg[(shift_pos+nr_shift_regs-1)%nr_shift_regs])+', %%'+str(temp_reg)+';'
                                comment   = '// REG ops only'
                                mov_dst = mov_dst +1
                            elif item == 'L1_L':
                                d0_inst   = 'vfmaddpd %%xmm'+str(add_start+(add_dest-add_start+add_regs+1)%add_regs)+', %%xmm0, %%xmm'+str(add_dest)+', %%xmm'+str(add_dest)+';'
                                d1_inst   = 'vfmaddpd 32(%%'+l1_addr+'), %%ymm1, %%ymm'+str(add_dest)+', %%ymm'+str(add_dest)+';'
                                ...
                                d1_inst   = 'vfmaddpd %%ymm'+str(add_start+(add_dest-add_start+add_regs+1)%add_regs)+', %%ymm0, %%ymm'+str(add_dest)+', %%ymm'+str(add_dest)+';'
                                l1_offset = l1_offset + each.cl_size
                                if l1_offset < l1_size*each.l1_cover:
                                    d3_inst  = 'add %%'+offset_reg+', %%'+l1_addr+';'
                                else:
                                ...
                                d1_inst   = 'vfmaddpd 32(%%'+l1_addr+'), %%ymm0, %%ymm'+str(add_dest)+', %%ymm'+str(add_dest)+';'
                                l1_offset = l1_offset + each.cl_size
                                if l1_offset < l1_size*each.l1_cover:
                                    d3_inst  = 'add %%'+offset_reg+', %%'+l1_addr+';'
                                else:
                                ...
                                d0_inst   = 'vfmaddpd %%xmm'+str(add_start+(add_dest-add_start+add_regs+1)%add_regs)+', %%xmm0, %%xmm'+str(add_dest)+', %%xmm'+str(add_dest)+';'
                                d1_inst   = 'vfmaddpd 64(%%'+l2_addr+'), %%xmm1, %%xmm'+str(add_dest)+', %%xmm'+str(add_dest)+';'
                                d3_inst   = 'add %%'+str(offset_reg)+', %%'+l2_addr+';'
                                comment   = '// L2 load'
                            elif item == 'L2_S':
                                d0_inst   = 'vmovapd %%xmm'+str(add_dest)+', 64(%%'+l2_addr+');'
                                d1_inst   = 'vfmaddpd %%xmm'+str(add_start+(add_dest-add_start+add_regs+1)%add_regs)+', %%xmm0, %%xmm'+str(add_dest)+', %%xmm'+str(add_dest)+';'
                                d3_inst   = 'add %%'+str(offset_reg)+', %%'+l2_addr+';'
                                comment   = '// L2 store'
                            elif item == 'L2_LS':
                                d0_inst   = 'vmovapd %%xmm'+str(add_dest)+', 96(%%'+l2_addr+');'
                                d1_inst   = 'vfmaddpd 64(%%'+l2_addr+'), %%xmm0, %%xmm'+str(add_dest)+', %%xmm'+str(add_dest)+';'
                                d3_inst   = 'add %%'+str(offset_reg)+', %%'+l2_addr+';'
                                comment   = '// L2 load, L2 store'
                            elif item == 'L3_L':
                                d0_inst   = 'vfmaddpd %%xmm'+str(add_start+(add_dest-add_start+add_regs+1)%add_regs)+', %%xmm0, %%xmm'+str(add_dest)+', %%xmm'+str(add_dest)+';'
                                d1_inst   = 'vfmaddpd 64(%%'+l3_addr+'), %%xmm1, %%xmm'+str(add_dest)+', %%xmm'+str(add_dest)+';'
                                ...
                                d0_inst   = 'vmovapd %%xmm'+str(add_dest)+', 96(%%'+l3_addr+');'
                                d1_inst   = 'vfmaddpd 64(%%'+l3_addr+'), %%xmm0, %%xmm'+str(add_dest)+', %%xmm'+str(add_dest)+';'
                                d3_inst   = 'add %%'+str(offset_reg)+', %%'+l3_addr+';'
                                comment   = '// L3 load, L3 store'
                            elif item == 'L3_P':
                                d0_inst   = 'vfmaddpd 32(%%'+l1_addr+'), %%xmm0, %%xmm'+str(add_dest)+', %%xmm'+str(add_dest)+';'
                                d1_inst   = 'prefetcht2 (%%'+l3_addr+');'
                                d3_inst   = 'add %%'+str(offset_reg)+', %%'+l3_addr+';'
                                comment   = '// L3 prefetch'
                            elif item == 'RAM_L':
                                d0_inst   = 'vfmaddpd %%xmm'+str(add_start+(add_dest-add_start+add_regs+1)%add_regs)+', %%xmm0, %%xmm'+str(add_dest)+', %%xmm'+str(add_dest)+';'
                                d1_inst   = 'vfmaddpd 64(%%'+ram_addr+'), %%xmm1, %%'+str(ram_reg)+', %%'+str(ram_reg)+';'
                                ...
                                d0_inst   = 'vmovapd %%xmm'+str(add_dest)+', 64(%%'+ram_addr+');'
                                d1_inst   = 'vfmaddpd 32(%%'+ram_addr+'), %%xmm0, %%xmm'+str(add_dest)+', %%xmm'+str(add_dest)+';'
                                d3_inst   = 'add %%'+str(offset_reg)+', %%'+ram_addr+';'
