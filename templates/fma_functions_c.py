...
#                    file.write("    for (i = 0; i<"+str(buffersize)+"; i++) ((double*)addrMem)[i] = 0.25 + (double)(i&0x1FFF) * 0.27948995982e-4;\n")
                    ...
                    for i in range(0,len(each.instr_groups)):
                        ...
                        if each.instr_groups[i] == 'REG':
                            flops=16 # two 256 bit FMA operations
                        elif each.instr_groups[i] == 'L1_L':
                            flops=16 # two 256 bit FMA operations
                        elif each.instr_groups[i] == 'L1_2L':
                            flops=16 # two 256 bit FMA operations
                        elif each.instr_groups[i] == 'L1_S':
                            flops=8 # one 256 bit FMA operation
                        elif each.instr_groups[i] == 'L1_LS':
                            flops=8 # one 256 bit FMA operation
                        elif each.instr_groups[i] == 'L1_LS_256':
                            flops=8 # one 256 bit FMA operation
                        elif each.instr_groups[i] == 'L1_2LS_256':
                            flops=16 # two 256 bit FMA operations
                            ...
                            flops=8 # one 256 bit FMA operation
                        elif each.instr_groups[i] == 'L2_LS_256':
                            flops=8 # one 256 bit FMA operation
                        elif each.instr_groups[i] == 'L2_2LS_256':
                            flops=16 # two 256 bit FMA operations
                            ...
                            flops=8 # one 256 bit FMA operation
                        elif each.instr_groups[i] == 'L3_LS_256':
                            flops=8 # one 256 bit FMA operation
            ...
            if item == isa:
                for threads in each.threads:
                    ...
                    for i in range(add_start,trans_end+1):
                        file.write("        \"vmovapd "+str(256+i*32)+"(%%rax), %%ymm"+str(i)+";\"\n")
                    ...
                    for i in range(0,util.repeat(sequence,lines)):
                        for item in sequence:
                            ...
                            if item == 'REG':
                                ...
                                d0_inst   = 'vfmadd231pd %%ymm'+str(add_start+(add_dest-add_start+add_regs+1)%add_regs)+', %%ymm0, %%ymm'+str(add_dest)+';'
                                d1_inst   = 'vfmadd231pd 32(%%'+l1_addr+'), %%ymm1, %%ymm'+str(add_dest)+';'
                                ...
                                d0_inst   = 'vfmadd231pd 32(%%'+l1_addr+'), %%ymm0, %%ymm'+str(add_dest)+';'
                                d1_inst   = 'vfmadd231pd 64(%%'+l1_addr+'), %%ymm1, %%ymm'+str(mov_dst)+';'
                                l1_offset = l1_offset + each.cl_size
                                if l1_offset < l1_size*each.l1_cover:
                                    d3_inst  = 'add %%'+offset_reg+', %%'+l1_addr+';'
                                else:
                                ...
                                d0_inst   = 'vmovapd %%xmm'+str(add_dest)+', 32(%%'+l1_addr+');'
                                d1_inst   = 'vfmadd231pd %%ymm'+str(add_start+(add_dest-add_start+add_regs+1)%add_regs)+', %%ymm0, %%ymm'+str(add_dest)+';'
                                l1_offset = l1_offset + each.cl_size
                                if l1_offset < l1_size*each.l1_cover:
                                    d3_inst  = 'add %%'+offset_reg+', %%'+l1_addr+';'
                                else:
                                ...
                                d0_inst   = 'vmovapd %%ymm'+str(add_dest)+', 64(%%'+l1_addr+');'
                                d1_inst   = 'vfmadd231pd 32(%%'+l1_addr+'), %%ymm0, %%ymm'+str(add_dest)+';'
                                l1_offset = l1_offset + each.cl_size
                                if l1_offset < l1_size*each.l1_cover:
                                    d3_inst  = 'add %%'+offset_reg+', %%'+l1_addr+';'
                                    ...
                                    d3_inst  = 'mov %%'+pointer_reg+', %%'+l1_addr+';'
                                comment   = '// L1 load, L1 store'
                            elif item == 'L1_2LS_256':
                                d0_inst   = 'vfmadd231pd 64(%%'+l1_addr+'), %%ymm0, %%ymm'+str(add_dest)+';'
                                d1_inst   = 'vfmadd231pd 96(%%'+l1_addr+'), %%ymm1, %%ymm'+str(mov_dst)+';'
                                d2_inst   = 'vmovapd %%ymm'+str(add_dest)+', 32(%%'+l1_addr+');'
                                l1_offset = l1_offset + 2* each.cl_size
                                if l1_offset < l1_size*each.l1_cover:
                                    d3_inst  = 'add $'+str(2* each.cl_size)+', %%'+l1_addr+';'
                                else:
                                    l1_offset = 0
                                    d3_inst  = 'mov %%'+pointer_reg+', %%'+l1_addr+';'
                                comment   = '// 2 L1 loads, L1 store'
                            elif item == 'L2_L':
                                d0_inst   = 'vfmadd231pd %%ymm'+str(add_start+(add_dest-add_start+add_regs+1)%add_regs)+', %%ymm0, %%ymm'+str(add_dest)+';'
                                d1_inst   = 'vfmadd231pd 64(%%'+l2_addr+'), %%ymm1, %%ymm'+str(add_dest)+';'
                                d3_inst   = 'add %%'+str(offset_reg)+', %%'+l2_addr+';'
                                comment   = '// L2 load'
                            elif item == 'L2_S':
                                d0_inst   = 'vmovapd %%xmm'+str(add_dest)+', 64(%%'+l2_addr+');'
                                d1_inst   = 'vfmadd231pd %%ymm'+str(add_start+(add_dest-add_start+add_regs+1)%add_regs)+', %%ymm0, %%ymm'+str(add_dest)+';'
                                d3_inst   = 'add %%'+str(offset_reg)+', %%'+l2_addr+';'
                                comment   = '// L2 store'
                            elif item == 'L2_LS':
                                d0_inst   = 'vmovapd %%xmm'+str(add_dest)+', 96(%%'+l2_addr+');'
                                d1_inst   = 'vfmadd231pd 64(%%'+l2_addr+'), %%ymm0, %%ymm'+str(add_dest)+';'
                                d3_inst   = 'add %%'+str(offset_reg)+', %%'+l2_addr+';'
                                comment   = '// L2 load, L2 store'
                            elif item == 'L2_LS_256':
                                d0_inst   = 'vmovapd %%ymm'+str(add_dest)+', 96(%%'+l2_addr+');'
                                d1_inst   = 'vfmadd231pd 64(%%'+l2_addr+'), %%ymm0, %%ymm'+str(add_dest)+';'
                                d3_inst   = 'add %%'+str(offset_reg)+', %%'+l2_addr+';'
                                comment   = '// L2 load, L2 store'
                            elif item == 'L2_2LS_256':
                                d0_inst   = 'vfmadd231pd 64(%%'+l2_addr+'), %%ymm0, %%ymm'+str(add_dest)+';'
                                d1_inst   = 'vfmadd231pd 96(%%'+l2_addr+'), %%ymm1, %%ymm'+str(mov_dst)+';'
                                ...
                                d0_inst   = 'vmovapd %%xmm'+str(add_dest)+', 96(%%'+l3_addr+');'
                                d1_inst   = 'vfmadd231pd %%ymm'+str(add_start+(add_dest-add_start+add_regs+1)%add_regs)+', %%ymm0, %%ymm'+str(add_dest)+';'
                                d3_inst   = 'add %%'+str(offset_reg)+', %%'+l3_addr+';'
                                comment   = '// L3 store'
                            elif item == 'L3_LS':
                                d0_inst   = 'vmovapd %%xmm'+str(add_dest)+', 96(%%'+l3_addr+');'
                                d1_inst   = 'vfmadd231pd 64(%%'+l3_addr+'), %%ymm0, %%ymm'+str(add_dest)+';'
                                d3_inst   = 'add %%'+str(offset_reg)+', %%'+l3_addr+';'
                                comment   = '// L3 load, L3 store'
                            elif item == 'L3_LS_256':
                                d0_inst   = 'vmovapd %%ymm'+str(add_dest)+', 96(%%'+l3_addr+');'
                                d1_inst   = 'vfmadd231pd 64(%%'+l3_addr+'), %%ymm0, %%ymm'+str(add_dest)+';'
                                d3_inst   = 'add %%'+str(offset_reg)+', %%'+l3_addr+';'
                                comment   = '// L3 load, L3 store'
                            elif item == 'L3_P':
                                ...
                                d0_inst   = 'vfmadd231pd %%ymm'+str(add_start+(add_dest-add_start+add_regs+1)%add_regs)+', %%ymm0, %%ymm'+str(add_dest)+';'
                                d1_inst   = 'vfmadd231pd 64(%%'+ram_addr+'), %%ymm1, %%'+str(ram_reg)+';'
                                d3_inst   = 'add %%'+str(offset_reg)+', %%'+ram_addr+';'
                                comment   = '// RAM load'
                            elif item == 'RAM_S':
                                d0_inst   = 'vmovapd %%xmm'+str(add_dest)+', 64(%%'+ram_addr+');'
                                d1_inst   = 'vfmadd231pd %%ymm'+str(add_start+(add_dest-add_start+add_regs+1)%add_regs)+', %%ymm0, %%ymm'+str(add_dest)+';'
                                d3_inst   = 'add %%'+str(offset_reg)+', %%'+ram_addr+';'
                                comment   = '// RAM store'
                            elif item == 'RAM_LS':
                                d0_inst   = 'vmovapd %%xmm'+str(add_dest)+', 64(%%'+ram_addr+');'
                                d1_inst   = 'vfmadd231pd 32(%%'+ram_addr+'), %%ymm0, %%ymm'+str(add_dest)+';'
                                d3_inst   = 'add %%'+str(offset_reg)+', %%'+ram_addr+';'
