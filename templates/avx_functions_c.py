        ...
        for item in each.isa:
            if item == isa:
                for threads in each.threads:
                    ...
                    file.write("         *   - {:<14} stores original pointer to buffer, used to periodically reset other pointers\n".format(pointer_reg+":"))
                    file.write("         *   - {:<14} pointer to L1 buffer\n".format(l1_addr+":"))
                    file.write("         *   - {:<14} pointer to L2 buffer\n".format(l2_addr+":"))
                    file.write("         *   - {:<14} pointer to L3 buffer\n".format(l3_addr+":"))
                    file.write("         *   - {:<14} pointer to RAM buffer\n".format(ram_addr+":"))
                    file.write("         *   - {:<14} counter for L2-pointer reset\n".format(l2_count_reg+":"))
                    file.write("         *   - {:<14} counter for L3-pointer reset\n".format(l3_count_reg+":"))
                    file.write("         *   - {:<14} counter for RAM-pointer reset\n".format(ram_count_reg+":"))
                    file.write("         *   - {:<14} register for temporary results\n".format(temp_reg+":"))
                    file.write("         *   - {:<14} stores cacheline width as increment for buffer addresses\n".format(offset_reg+":"))
                    file.write("         *   - {:<14} stores address of shared variable that controls load level\n".format(addrHigh_reg+":"))
                    file.write("         *   - {:<14} stores iteration counter\n".format(iter_reg+":"))
                    file.write("         *   - {:<14} data registers for SIMD instructions\n".format("mm*,xmm*,ymm*:"))
                    # file.write("         *\n")
                    ...
                    if add_regs > 0:
                        ...
                        file.write("        //Initialize AVX-Registers for Addition\n")
                        for i in range(add_start,add_end+1):
                            file.write("        \"vmovapd "+str(i*32)+"(%%rax), %%ymm"+str(i)+";\"\n")
                    if shift_regs > 1:
                        shift_start = 0
                        shift_end   = shift_regs-1
                        file.write("        //Initialize MMX-Registers for shift operations\n")
                        file.write("        \"movabs $0x5555555555555555, %%"+temp_reg+";\"\n")
                        file.write("        \"movq %%"+temp_reg+", %%mm"+str(shift_start)+";\"\n")
                        for i in range(shift_start+1,shift_end+1):
                            file.write("        \"movq %%mm"+str(shift_start)+", %%mm"+str(i)+";\"\n")
                    if trans_regs > 0:
                        file.write("        //Initialize AVX-Registers for Transfer-Operations\n")
                        if trans_start % 2 == 0:
                        ...
                        file.write("        \"vinsertf128 $1, %%xmm"+str(trans_start)+", %%ymm"+str(trans_start)+", %%ymm"+str(trans_start)+";\"\n")
                        for i in range(trans_start+1,trans_end+1):
                            ...
                            file.write("        \"vinsertf128 $1, %%xmm"+str(i)+", %%ymm"+str(i)+", %%ymm"+str(i)+";\"\n")
                    ...
                    for i in range(0,util.repeat(sequence,lines)):
                        for item in sequence:
                            ...
                            if item == 'REG':
                                d0_inst   = 'vaddpd %%ymm'+str(add_start+(add_dest-add_start+add_regs-1)%add_regs)+', %%ymm0, %%ymm'+str(add_dest)+';'
                                d1_inst   = ''
                                d3_inst   = 'vmovdqa %%ymm'+str(mov_src)+', %%ymm'+str(mov_dst)+';'
                                comment   = '// REG ops only'
                            elif item == 'L1_L':
                                d0_inst   = 'vaddpd 32(%%'+l1_addr+'), %%ymm'+str(add_dest)+', %%ymm'+str(add_dest)+';'
                                d1_inst   = ''
                                ...
                                d0_inst   = 'vaddpd %%ymm'+str(add_start+(add_dest-add_start+add_regs-1)%add_regs)+', %%ymm'+str(add_dest)+', %%ymm'+str(add_dest)+';'
                                d1_inst   = 'vmovapd %%xmm'+str(add_dest)+', 32(%%'+l1_addr+');'
                                l1_offset = l1_offset + each.cl_size
                                if l1_offset < l1_size*each.l1_cover:
                                    d3_inst  = 'add %%'+offset_reg+', %%'+l1_addr+';'
                                else:
                                ...
                                d0_inst   = 'vaddpd 32(%%'+l1_addr+'), %%ymm'+str(add_dest)+', %%ymm'+str(add_dest)+';'
                                d1_inst   = 'vmovapd %%xmm'+str(add_dest)+', 64(%%'+l1_addr+');'
                                l1_offset = l1_offset + each.cl_size
                                if l1_offset < l1_size*each.l1_cover:
                                    d3_inst  = 'add %%'+offset_reg+', %%'+l1_addr+';'
                                else:
                                ...
                                d0_inst   = 'vaddpd 64(%%'+l2_addr+'), %%ymm'+str(add_dest)+', %%ymm'+str(add_dest)+';'
                                d1_inst   = ''
                                d3_inst   = 'add %%'+offset_reg+', %%'+l2_addr+';'
                                comment   = '// L2 load'
                            elif item == 'L2_S':
                                d0_inst   = 'vaddpd %%ymm'+str(add_start+(add_dest-add_start+add_regs-1)%add_regs)+', %%ymm'+str(add_dest)+', %%ymm'+str(add_dest)+';'
                                d1_inst   = 'vmovapd %%xmm'+str(add_dest)+', 64(%%'+l2_addr+');'
                                d3_inst   = 'add %%'+offset_reg+', %%'+l2_addr+';'
                                comment   = '// L2 store'
                            elif item == 'L2_LS':
                                d0_inst   = 'vaddpd 64(%%'+l2_addr+'), %%ymm'+str(add_dest)+', %%ymm'+str(add_dest)+';'
                                d1_inst   = 'vmovapd %%xmm'+str(add_dest)+', 96(%%'+l2_addr+');'
                                d3_inst   = 'add %%'+offset_reg+', %%'+l2_addr+';'
                                comment   = '// L2 load, L2 store'
                            elif item == 'L3_L':
                                d0_inst   = 'vaddpd 64(%%'+l3_addr+'), %%ymm'+str(add_dest)+', %%ymm'+str(add_dest)+';'
                                d1_inst   = ''
                                d3_inst   = 'add %%'+str(offset_reg)+', %%'+l3_addr+';'
                                comment   = '// L3 load'
                            elif item == 'L3_S':
                                d0_inst  = 'vaddpd %%ymm'+str(add_start+(add_dest-add_start+add_regs-1)%add_regs)+', %%ymm'+str(add_dest)+', %%ymm'+str(add_dest)+';'
                                d1_inst  = 'vmovapd %%xmm'+str(add_dest)+', 96(%%'+l3_addr+');'
                                d3_inst   = 'add %%'+str(offset_reg)+', %%'+l3_addr+';'
                                comment  = '// L3 store'
                            elif item == 'L3_LS':
                                d0_inst  = 'vaddpd 64(%%'+l3_addr+'), %%ymm'+str(add_dest)+', %%ymm'+str(add_dest)+';'
                                ...
                                d0_inst  = 'vaddpd 64(%%'+ram_addr+'), %%ymm'+str(add_dest)+', %%ymm'+str(add_dest)+';'
                                d1_inst   = ''
                                d3_inst  = 'add %%'+str(offset_reg)+', %%'+ram_addr+';'
                                comment  = '// RAM load'
                            elif item == 'RAM_S':
                                d0_inst  = 'vaddpd %%ymm'+str(add_start+(add_dest-add_start+add_regs-1)%add_regs)+', %%ymm'+str(add_dest)+', %%ymm'+str(add_dest)+';'
                                d1_inst  = 'vmovapd %%xmm'+str(add_dest)+', 64(%%'+ram_addr+');'
                                d3_inst  = 'add %%'+str(offset_reg)+', %%'+ram_addr+';'
                                comment  = '// RAM store'
                            elif item == 'RAM_LS':
                                d0_inst  = 'vaddpd 64(%%'+l3_addr+'), %%ymm'+str(add_dest)+', %%ymm'+str(add_dest)+';'
                            ...
                            if shift_regs > 1:
                                if dir %2 == 0:
                                    d2_inst = 'psllw %%mm'+str(shift_dest)+', %%mm'+str(shift_start+(shift_dest-shift_start+3)%shift_regs)+';'
                                else:
                                    d2_inst = 'psrlw %%mm'+str(shift_dest)+', %%mm'+str(shift_start+(shift_dest-shift_start+3)%shift_regs)+';'
                            else:
                                d2_inst = ''
                            # write instruction group to file
                            file.write("        \"{:<35} {:<35} {:<35} {:<35}\" {:<}\n".format(d0_inst, d1_inst, d2_inst, d3_inst, comment))
                            ...
                            if mov_src > trans_end:
                                mov_src = trans_start
                            if shift_regs > 1:
                                shift_dest = shift_dest +1
                                if shift_dest > shift_end:
                                    shift_dest = shift_start
                                    dir = dir +1 %2
                        count = count + 1
